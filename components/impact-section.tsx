    ...
    <section className="w-full font-sans border-y border-border overflow-hidden">
      ...
      */}
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[580px] w-full">
        ...
        <div className="lg:col-span-6 bg-primary text-primary-foreground relative p-8 md:p-12 lg:p-16 flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-2">
              {/* Description column */}
              <div className="md:col-span-6 space-y-4 text-primary-foreground/80 text-sm md:text-base leading-relaxed font-medium">
                <p>{description}</p>
                {pillarsTitle && (
                  <p className="font-bold text-primary-foreground pt-2">
                    {pillarsTitle}
                  </p>
                )}
              </div>
              ...
              <div className="md:col-span-6 space-y-2.5">
                {pillars.map((pillar, idx) => (
                  <div key={idx} className="flex items-center gap-3 group">
                    <span className="flex-shrink-0 text-accent bg-accent/15 rounded-full p-0.5 group-hover:scale-110 transition-transform">
            ...
            <div className="pt-4">
              <Link href={ctaLink} className="inline-block">
                <button className="bg-accent hover:bg-accent/90 text-accent-foreground font-extrabold text-xs md:text-sm tracking-wider uppercase px-8 py-4 rounded-none shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 flex items-center gap-2 cursor-pointer">
                  <span>{ctaText}</span>
                </button>
              </Link>
            </div>
        ...
        {/* MIDDLE STATS CONTAINER (Theme Adaptable: Secondary Light in Light Mode / Card in Dark Mode) */}
        <div className="lg:col-span-3 bg-secondary text-secondary-foreground dark:bg-card dark:text-card-foreground p-8 md:p-12 flex flex-col justify-center gap-10 border-t lg:border-t-0 lg:border-l border-border">
          {stats.map((stat, idx) => (
            <div key={idx} className="space-y-3">
              <div className="text-4xl sm:text-5xl lg:text-6xl font-black text-accent tracking-tight">
                {stat.value}
              </div>
